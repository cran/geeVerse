...
#' @param cutoff Integer, the threshold to determine whether a esimated beta should
#' be consider as 0.
#'
...
#'
#' @export
compile_result <- function(qpgee_results,beta0,cutoff = 0.1){
  p = length(beta0)
  if(inherits(qpgee_results,"qpgee")){qpgee_results = list(qpgee_results)}
  ...
  for(sim in 1:n_sim){
    ...
    selected <- abs(qpgee_results[[sim]]$beta)>cutoff
...
#compile results
# compile_result_hd <- function(qpgee_results,SIS_results,beta0,cutoff = 0.1){
...
#     selected <- rep(FALSE,p)
#     selected[SIS_X] <- abs(qpgee_results[[sim]]$beta)>cutoff
#     selected_true <- abs(beta0)>cutoff
#     correct <- FALSE
#     TP <- sum(selected & selected_true)
#     FP <- sum(selected & !selected_true)
